...
### UniversitÃ  degli Studi di Milano | Single cell analysis boot camp
### 25/07/2024
...
setwd("//unicloud.unimi.it/share/Home/cristina.vicenzetto/Desktop/NicheNet_esercitazione/Data")
#..
...
head(MAIT@meta.data)# per vedere i metadati
...
## Ligand-receptor network: a set of potential ligands and receptors with source-database information, è una lista generale vale per tutte le cellule è una lista generale di predizioni
...
## Sender: MAIT cells
Idents(MAIT) = 'time'#to select the mait at t3
expressed_genes_sender = get_expressed_genes("P3", MAIT, pct = 0.10)#impute cel object, tyo select all gens present at least in 10% of cells (the most expressed genes) at the time point we can check the first 
...
B_P3_c2 = subset(B_P3, idents = '2')#per selezionare un sottotipo del cluster dell'ident cluster

Idents(B_P3_c2) = 'time'#we would to select b cells at p3
...
head(background_expressed_genes)#per visualizzare i primi geni

### Define a gene set of interest
  # These are the genes in the â€œreceiver/targetâ€ cell population that are differently expressed between the baseline (P0/pre-vaccine)
...
DEG = read.xlsx("DEGs Bc2_P3vsP0.xlsx", rowNames = TRUE, colNames = TRUE)#logfolf chage dopra 1 upredolato
head(DEG)
length(DEG$names)

## Only significant DEGs have to be considered in the analysis:
DEG_filter = filter(DEG, pvals_adj <= 0.05 & abs(logfoldchanges) >= 0.25  & pct_nz_group > 0.1)#seleziono i DEG signif con una fùdiff del 25% e espressi almenp nel 10% delle cellule (per ridurre i falsi positivi)
head(DEG_filter)
length(DEG_filter$names)

rownames(DEG_filter) = DEG_filter$names#we take the name to use them as gen of interest
...
ligands =  lr_network %>% pull(from) %>% unique()#prendo solo la colonna dei ligandi, funzione:pull
receptors = lr_network %>% pull(to) %>% unique()

expressed_ligands = intersect(ligands, expressed_genes_sender)#intersezione dei ligandi con i sender, quindi solo i ligandi del sender
expressed_receptors =  intersect(receptors, expressed_genes_receiver)

potential_ligands =  lr_network %>% filter(from %in% expressed_ligands & to %in% expressed_receptors) %>% pull(from) %>% unique()#select all ligand che hanno un ligando che è stato filtrato che sono stati trovati ne
...
#funct: predice_ligand_sctivaties, servono imput:
#i geni deg
#backgroud
#i geni ligandi nei B
#quelli che possono interegire con mail

ligand_activities = predict_ligand_activities(geneset = geneset_oi, background_expressed_genes = background_expressed_genes, ligand_target_matrix = ligand_target_matrix, potential_ligands = potential_ligands)
ligand_activities = ligand_activities %>% arrange(-pearson) %>% mutate(rank = rank(desc(pearson)))#oerder from the lower value to the highest value
...
#take deg and select onlygene that interact with legands and results, select the best downstream gene for the ligands identified,quindi quali vie attivano, il weight dice la forza dell'interazione dalla matrice di partenza da letteratura
active_ligand_target_links_df = best_upstream_ligands %>% lapply(get_weighted_ligand_target_links, geneset = geneset_oi,
                                                                 ligand_target_matrix = ligand_target_matrix, n = 200) %>% bind_rows() %>% drop_na()#n si sceglie quanti sono i gweni regolati
